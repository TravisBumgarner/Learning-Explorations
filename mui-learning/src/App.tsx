...
type AudioStreamDetails = {
	...
	settings: Record<Keys, MediaTrackSettings>;
	capabilities: Record<Keys, MediaTrackCapabilities>;
};
...
const DebugOverlay = ({ analysers, settings }: AudioStreamDetails) => {
	...
	useEffect(() => {
		...
		console.log("rudaa", microphoneDataArray);
	...
	return (
		<DebugOverlayWrapper>
			<div>
				...
				<div>{listify(settings.microphone)}</div>
			</div>
			<div>
				<h1>Display</h1>
				<canvas ref={displayRef} />
				<div>{listify(settings.display)}</div>
			</div>
			<div>
				<h1>Output</h1>
				<canvas ref={outputRef} />
				<div>{listify(settings.output)}</div>
			</div>
		</DebugOverlayWrapper>
	);
};
...
const App = () => {
	...
	const [audioStreamDetails, setAudioStreamDetails] =
		useState<AudioStreamDetails | null>(null);
	...
	const mergeAudioStreams = useCallback(
		(userStream: MediaStream, displayStream: MediaStream) => {
			audioContext.current = new AudioContext();

			const mergedAudioStreamDestinationNode =
				audioContext.current.createMediaStreamDestination();

			const displayAudio =
				audioContext.current.createMediaStreamSource(displayStream);
			const displayAnalyser = new AnalyserNode(audioContext.current, {
				fftSize: 256,
			});
			displayAudio
			...
			const userAudio =
				audioContext.current.createMediaStreamSource(userStream);
			gainNode.current = new GainNode(audioContext.current, {
				gain: isMuted ? 0 : 0.5,
			});
			const microphoneAnalyser = new AnalyserNode(audioContext.current, {
				fftSize: 256,
			});
			userAudio
			...
			const mergedAudioTrack =
				mergedAudioStreamDestinationNode.stream.getAudioTracks()[0];
			const mergedAudioStream = new MediaStream([mergedAudioTrack]);

			const toMonitor =
				audioContext.current.createMediaStreamSource(mergedAudioStream);
			const outputAnalyser = new AnalyserNode(audioContext.current, {
				fftSize: 256,
			});
			toMonitor.connect(outputAnalyser);

			setAudioStreamDetails((prev) => ({
				...prev,
				analysers: {
					microphone: microphoneAnalyser,
					display: displayAnalyser,
	...
	const getMediaStream = useCallback(async () => {
		const userStream = await globalThis.navigator.mediaDevices.getUserMedia({
			audio: true,
		});
		const displayStream =
