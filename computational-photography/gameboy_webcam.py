...
while True:
    ...
    color_buckets = grayscale_buckets[:, :, np.newaxis] * np.ones(
        (HEIGHT, WIDTH, 3)
    ).astype(np.uint8)

    color_buckets = np.where(
        color_buckets == [0, 0, 0], np.array([155, 188, 15]), color_buckets
    )
    color_buckets = np.where(
        color_buckets == [1, 1, 1], np.array([48, 246, 35]), color_buckets
    )
    color_buckets = np.where(
        color_buckets == [2, 2, 2], np.array([139, 172, 15]), color_buckets
    )
    color_buckets = np.where(
        color_buckets == [3, 3, 3], np.array([15, 56, 15]), color_buckets
    )
    ...
    if counter == FRAMES_TO_COUNT:
        ...
        print(
            f"{FRAMES_TO_COUNT} frames took {end-start}s, or {FRAMES_TO_COUNT / (end-start)} FPS"
        )
        counter = 0
